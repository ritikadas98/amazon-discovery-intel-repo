...
# Where the dashboard lives. Separate from the Cloud Run URL, which is the API —
# digest links and the post-feedback page send readers here, not to a JSON host.
export APP_BASE_URL="${APP_BASE_URL:-https://amazon.ritikadas.in}"
...
# Owner override for the 24-hour reuse window. Never hardcode it here — pass it in:
#   PIPELINE_FORCE_TOKEN='some-long-random-string' bash scripts/gcp-deploy.sh
# Left unset, the override does not exist and every caller waits out the window.
export PIPELINE_FORCE_TOKEN="${PIPELINE_FORCE_TOKEN:-}"
